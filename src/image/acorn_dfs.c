 ...
 * acorn.c
 * 
 * Acorn DFS SSD and DSD Single Density 80 Track FM Disks
 * 80 Tracks 10 256 Byte Sectors per Track
 * 
...
{
    return _dfs_open(im, TRUE);
}

static bool_t dfs_seek_track(
...
static bool_t dfs_read_track(struct image *im)
{
    struct image_buf *rd = &im->bufs.read_data;
    struct image_buf *mfm = &im->bufs.read_mfm;
    uint8_t *buf = rd->p;
...
#define emit_raw(r) ({                                  \
    ...
    pr = _r; })
#define emit_byte(b) emit_raw(fmtab[(uint8_t)(b)])
...
static uint16_t dfs_rdata_flux(struct image *im, uint16_t *tbuf, uint16_t nr)
{
...
static void dfs_write_track(struct image *im, bool_t flush)
{
